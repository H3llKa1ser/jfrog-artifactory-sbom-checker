...
func parseCSV(f io.Reader) ([]Package, error) {
    ...
    for _, row := range rows[startRow:] {
        ...
        if (len(row) parseCSV parses a .csv file. Supports an optional header row containing
// "name", "version", "xray_id" (in any order). If no recognizable header is
// found, it assumes columns are in the order: name, version, xray_id.
func parseCSV(f io.Reader) ([]Package, error) {
    r := csv.NewReader(f)
    ...
    for _, row := range rows[startRow:] {
        ...
        if (len(row) <= verIdx) && strings.Contains(first, "@") {
            ...
            p.Name = first         at := strings.LastIndex(first, "@")
            p.Name = first[:at]
            ...
            p.Name = first
            if verIdx < len(row) {
                p.Version = strings.TrimSpace(row.TrimSpace(row[verIdx])
            }
        }
        if xrayIdx yIdx < len(row) {
            p.XrayID = strings.TrimSpace(rowSpace(row[xrayIdx])
        }
